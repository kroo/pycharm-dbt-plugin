                ...
                object : CompletionProvider<CompletionParameters?>() {
                    override fun addCompletions(parameters: CompletionParameters,
                        ...
                        val isInsideFunctionCall = parameters.position.parent is DjangoVariableReferenceImpl
                                ...
                                && (parameters.position.parent.parent as Jinja2FunctionCall).callee is DjangoVariableReferenceImpl
                        val isAfterPartialFunctionCall = parameters.position.parent.nextSibling is PsiErrorElement &&
                                ((parameters.position.parent.nextSibling as PsiErrorElement).errorDescription == "Closing parenthesis expected")
                        val isFunctionCall = isInsideFunctionCall || isAfterPartialFunctionCall

                        val outerFunctionCallRef = when {
                            isInsideFunctionCall ->
                                ((parameters.position.parent.parent as Jinja2FunctionCall).callee as DjangoVariableReferenceImpl)
                        ...
                        if (isFunctionCall && outerFunctionCallRef?.name == "source") {
                            ...
                            val selectedParameterIndex = DbtJinja2FunctionParameterUtil.selectedParameterIndex(parameters.position, functionCall)
